...
class EMMA:
    ...
    def getElem(self):
        ...
        interp.setAttributeNS(
            self.namespaceEMMA, "emma:confidence", str(self.confidence)
        )
...
class LifeCycleEvent:
    ...
    def doNewContextRequest(self):
        mmi = self.doBaseMMI()
        newContextRequest = self._doc.createElementNS(
            self.namespaceMMI, "mmi:newContextRequest"
        )
    ...
    def doExtensionNotification(self, emma):
        mmi = self.doBaseMMI()
        ExtensionNotification = self._doc.createElementNS(
            self.namespaceMMI, "mmi:ExtensionNotification"
        )
...
class MMIClientSocket:
    ...
    async def openSocket(self):
        ...
        self.socket = await websockets.connect(
            self.address, ssl=ssl_context
        )  # ssl = ssl_context para ignorar certificado (menos seguro)
...
class MMIClient:
    ...
    def sendToIM(self, lce: LifeCycleEvent):
        # print(str(lce))
        response = httpx.post(
            self.FusionAdd, data=str(lce), verify=False
        )  # verify = False para ignorar certificado (menos seguro)
